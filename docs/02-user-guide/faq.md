...
`npx development-kit init --opencode` installs the compatible skills to `.opencode/skills/` plus `opencode.json` and `AGENTS.md`.
...
Yes. All compatible skills declare `compatibility: opencode` and OpenCode auto-discovers them from `.opencode/skills/`.
