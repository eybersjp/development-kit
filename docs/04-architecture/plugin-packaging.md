# Plugin Packaging

## What Is Packaged

`package.json` `files` field defines the npm package contents:

```json
[
  ".agents/", "agents/", "skills/", "commands/", "hooks/",
  "templates/", "evals/", "scripts/", "AGENTS.md", "README.md", "opencode.json"
]
```

The `bin` entry exposes `development-kit` → `scripts/install-antigravity.mjs`, enabling `npx development-kit init`.

## Plugin Structure (Antigravity)

An installed Antigravity plugin is self-contained at `<target>/plugins/development-kit/`:

```text
plugins/development-kit/
├── plugin.json        # manifest with ./ paths (rewritten from ../../../)
├── skills/            # copied
├── agents/            # copied
├── hooks/             # copied
└── commands/          # copied
```

## Packaging Flow

```mermaid
graph LR
    A["canonical root dirs"] -->|"npm pack (files field)"| N["npm tarball"]
    N -->|"npx development-kit init"| I["installer"]
    I -->|"copies + rewrites manifest paths"| P["<target>/plugins/development-kit/"]
    N -->|"npx development-kit init --opencode"| O[".opencode/skills/"]
```

## Manifest

`plugin.json` (`.agents/plugins/development-kit/plugin.json`) declares the plugin: `name`, `version` (`0.1.0`), and references for 43 skills, 18 agents, and 4 hooks. It is regenerated by `sync-plugin.mjs`.

## Version Alignment

- `package.json`: `0.3.0`
- npm tag: `v0.3.0`
- `plugin.json` and `hooks/session-start.js`: `0.1.0` (fixed; known inconsistency — see [known-limitations.md](../11-appendices/known-limitations.md))

## Publishing

Publishing is automated by `.github/workflows/publish.yml` on `v*` tags: validation, tag↔package version check, then `npm publish` with the `NPM_TOKEN` secret. See [npm-publishing.md](../08-maintenance-release/npm-publishing.md).
