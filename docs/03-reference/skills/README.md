...
| **Implementation** (8) | subagent-driven-implementation, incremental-implementation, live-ui-preview, test-driven-development, existing-code-first, native-platform-first, dependency-restraint, minimal-diff | IMPLEMENT |
...
- [`live-ui-preview`](live-ui-preview.md) arms and maintains the running frontend during UI/design work. It is development visibility; formal VERIFY-time browser evidence remains the responsibility of `browser-runtime-verification`.
...
All 48 engineering skills declare `compatibility: opencode` in their frontmatter and are auto-discoverable by OpenCode when installed to `.opencode/skills/` (`.opencode/skills` is one of OpenCode's discovery paths). In Antigravity they are loaded from the plugin's `skills/` directory.
