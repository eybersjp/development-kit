...
| `version` | `0.10.1` | Current package version and `v0.10.1` release |
...
New Autopilot workflow state records the framework release version in `frameworkVersion`. For v0.10.1 this value is `0.10.1`. The v0.9 version-consistency regression checks that this value remains aligned with both `package.json` and the plugin manifest.
