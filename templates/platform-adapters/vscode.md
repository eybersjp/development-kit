# Development Kit for GitHub Copilot
...
The Development Kit workflow entry points are `/dk-autopilot`, `/dk-idea`, `/dk-research`, `/dk-spec`, `/dk-design`, `/dk-tasks`, `/dk-build`, `/dk-build-auto`, `/dk-test`, `/dk-review`, `/dk-simplify`, `/dk-debug`, `/dk-ship`, `/dk-control`, and `/dk-status`. Treat these as workflow names when the current interface does not expose them as commands.
